    ...
    if (_G.profile.enable) {
        ...
        for (i = 0; i < MEMORY_SIZE; i++) {
            if (_G.profile.ticks[i] != 0) {
                char buffer[128];
                if(_G.profile.calls[i]!=0) {
                    fprintf(fp, "  %-25s %15lu %13lu %16lu %15.2f\n", map_lookup(_G.pmap, (uint16)i, buffer), 
                            _G.profile.ticks[i],
                            _G.profile.calls[i],
		    ...
		    fprintf(fp, "; cannot stat function %s\n", map_lookup(_G.pmap, (uint16)i, buffer));
		}
            }
        }
...
OPDEF(rst_08, 0xC7+8)
{
    ...
    case RST_08_PROFILE_ENTER_LEAF:
        ...
        break;
    case RST_08_DUMP_REGS:
	printf("Register dump\nPC: %04x   SP: %04x\nAF: %04x   HL: %04x\nBC: %04x   DE: %04x\nIX: %04x   IY: %04x\n",
		 z80_proc.mem[SP+2]|(z80_proc.mem[SP+3]<<8), SP, z80_proc.mem[SP]|(z80_proc.mem[SP+1]<<8), HL, BC, DE, IX, IY);
	fflush(stdout);
	break;
    default:
...
void debug_z80(void)
{
    if (_G.trace) {
        ...
        if (_G.traceRegs) {
            fprintf(stderr, "\tSP: %04X AF: %04X BC: %04X DE: %04X HL: %04X IX: %04X IY: %04X\n", SP, AF, BC, DE, HL, IX, IY);
        }
        else {
            fprintf(stderr, "\n");
        }
...
int main(int argc, char **argv)
{
    ...
    AF=0; BC=0; DE=0; HL=0; SP=0; PC=0; IX=0; IY=0;
