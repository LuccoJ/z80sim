...
short SameEnd(char* String1, char* String2) {
	int i, Offset;
	Offset=strlen(String1)-strlen(String2);
	for(i=0; i<strlen(String2); i++) {
		if(String1[i+Offset]!=String2[i]) return 0;
	}
	return 1;
} 
...
const char *map_lookup(MAP *self, uint16 addr, char *pInto)
{
    if (self != NULL) {
        int i;
        for (i = 0; i < self->numSyms; i++) {
            if (self->syms[i].addr == addr && !SameEnd(self->syms[i].szName, "_end") && !SameEnd(self->syms[i].szName, "_start")) {
		if(self->syms[i].szName[0]=='_') {
                    return &(self->syms[i].szName[1]);
		} else {
		    return self->syms[i].szName;
		}
            }
