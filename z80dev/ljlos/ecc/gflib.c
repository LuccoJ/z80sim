...
unsigned char gfmul(unsigned char mul1, unsigned char mul2) {
  ...
  else mul3 = e2v[(v2e[mul1] + v2e[mul2]) % 255];
  return (mul3);
}
...
unsigned char gfinv(unsigned char ivt) {
  unsigned char ivtd;
  // if(ivt == 0) return 0;   // THIS SHOULD RETURN AN ERROR INSTEAD
...
unsigned char gfexp(unsigned char mant, unsigned char powr) {
  unsigned char expt;
  if(mant == 0) expt = 0;
  else expt = e2v[(v2e[mant] * powr) % 255];
  return (expt);
}
